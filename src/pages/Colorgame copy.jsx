...
function Colorgame() {
  ...
  const handleImageClick = (index) => {
    const selectedBall = images[index]; // Ensure selectedBall is always an object
    console.log(
      `Clicked on image ${index + 1}, Colors: `,
      images[index].colors
    );
    setModalContent({
      index: index + 1, // Store ball number
      colors: selectedBall?.colors || [] // Ensure colors is always an array
    });
    setShowModal(true); // Open the modal
  };
  ...
  return (
    <>
      ...
      {showModal && (
        <Modal
          showModal={showModal}
          ballIndex={modalContent.index} // Pass index separately
          colors={modalContent.colors} // Pass colors separately
          onClose={closeModal}
